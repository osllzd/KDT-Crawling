...
def make_wordcloud(title_list, stopwords, word_count):
    ...
    img_mask = np.array(Image.open('AI11111.png'))
    wordcloud = WordCloud(font_path=path, width=800, height=600, background_color= 'black',
                   max_font_size=100, repeat=True, colormap='seismic', mask=img_mask)

    AI = wordcloud.generate_from_frequencies(tag_dict)
...
if __name__=='__main__':
    keywordList = ['AI기술','인공지능']

    for n in keywordList:
        search_word = n
        title_list = []
        stopwords = [search_word, 'AI']
        get_titles(1,100, search_word, title_list)

        print(title_list)


        with open('crawling'+ str(n) +'.csv', 'a', encoding='utf-8-sig', newline='') as f:
            writer = csv.writer(f)
            for t in title_list:
                writer.writerows([[t]])
            f.close()
